...
class BaseCollector:
  def __init__(
    ...
    self.current_ob, _ = self.env.reset()
    self.train_rew = 0
    ...
    for _ in range(self.eval_episodes):
      ...
      while not done:
        ...
        if self.continuous and np.isnan(act).any():
          ...
          exit()
        try:
          eval_ob, r, terminated, truncated, _ = self.eval_env.step(act)
    ...
    for _ in range(self.eval_episodes):
      ...
      while not np.all(epi_done):
        ...
        if self.continuous and np.isnan(act).any():
          ...
          exit()
        try:
          eval_obs, r, terminated, truncated, _ = self.eval_env.step(act)
